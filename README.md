...
```
## Транслятор
Реализовано в [translator.py](translator.py)
```
...
2. Транслятор проходит по каждому выражению из аст и транслирует его рекурсивно
   - Сначала транслируется код основной программы, затем функции языка (`read`, `input-string`, `nreverse`, `push`, `null`, `cdr`, `car`, `list`, `concat`, `print`), затем функции, объявленные через defun
...
4. Замена меток. После трансляции всех выражений метки заменяются на адресса. При этом внутри блоков переход проихсодит при помощи Pc + Offset, вызов через immediate value
...
## Модель процессора
Реализовано в [machine.py](machine.py)
```plaintext
